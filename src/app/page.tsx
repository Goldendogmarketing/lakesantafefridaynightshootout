...
function getLastWeekResults(): { week: TournamentWeek | null; results: Result[]; bigBass: Result | null } {
  const db = getDb();
  const week = db.prepare(
    'SELECT * FROM tournament_weeks WHERE is_upcoming = 0 ORDER BY date DESC LIMIT 1'
  ).get() as TournamentWeek | undefined;

  if (!week) return { week: null, results: [], bigBass: null };

  const results = db.prepare(
    'SELECT * FROM results WHERE week_id = ? ORDER BY placement ASC, total_weight DESC LIMIT 5'
  ...
  return { week, results, bigBass: bigBass || null };
}
...
export default function HomePage() {
  ...
  const { week: lastWeek, results: lastResults, bigBass } = getLastWeekResults();

  return (
    <div>
      {/* Hero Section - Sunset warm tones */}
      <section className="relative bg-gradient-to-br from-[#1a0e04] via-[#4a2008] to-[#7c3a0e] text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-28 text-center">
          ...
          <Image
            src="/logo.png"
            alt="Lake Santa Fe Friday Night Shoot Out"
            width={500}
            height={300}
            className="mx-auto mb-6 max-w-[90%] sm:max-w-md h-auto"
            priority
          />
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
            <Link
              href="/waiver"
              className="bg-[#c45e10] text-white px-8 py-4 rounded-lg text-lg font-bold hover:bg-[#e8940c] transition-colors shadow-lg shadow-orange-900/50"
            >
              Sign Waiver & Register
            </Link>
            <Link
              href="/results"
              className="bg-white/10 backdrop-blur text-white border border-[#f5b731]/40 px-8 py-4 rounded-lg text-lg font-semibold hover:bg-white/20 transition-colors"
            >
              View Results
            </Link>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-orange-100 overflow-hidden">
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-8 text-center">
              <Link
                href="/waiver"
                className="inline-block bg-[#c45e10] text-white px-8 py-3 rounded-lg font-semibold hover:bg-[#e8940c] transition-colors"
              >
        ...
        <section className="bg-[#2a1506]/5 py-12 sm:py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {lastResults.slice(0, 3).map((result, i) => (
                <div key={result.id} className="bg-white rounded-xl shadow-md p-6 text-center border border-orange-100">
                  ...
                  <div className="mt-3 flex justify-center gap-4 text-sm">
                    <span className="bg-orange-50 text-[#c45e10] px-3 py-1 rounded-full font-medium">
                      {result.total_weight} lbs
                    </span>
                    <span className="bg-amber-50 text-[#9a4a0d] px-3 py-1 rounded-full font-medium">
                      {result.num_fish} fish
                    </span>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center mt-8">
              <Link
                href={`/results/${lastWeek.id}`}
                className="text-[#c45e10] font-semibold hover:text-[#e8940c] transition-colors"
              >
